...
type OrderSummaryProps = {
  ...
  // если onContinue нет — навигация по умолчанию
  continueTo?: string;

  // текст кнопки (чтобы переиспользовать на Payment)
  continueLabel?: string;

  // можно отключать кнопку (например пока форма невалидна)
  continueDisabled?: boolean;

  // если нужно спрятать кнопку вообще
  ...
  return (
    <Box className="cart-right">
      ...
      <Box className="cart-total">
        ...
        <div className="cart-total-value">€ {amountOfProducts ? total.toFixed(2) : 0}</div>
      </Box>

      {showContinueButton && (
        <button className="cart-btn-payment" onClick={handleContinue} disabled={continueDisabled}>
          <img src={Lock} alt="Lock" /> {continueLabel}
        </button>
      )}
